...
def view_sig(args):
    ...
    spectrum = np.abs(fftshift(fft(iqsslicecomplex)))
    freqs = np.linspace(-0.5, 0.5, len(iqsslicecomplex))
    ax5.plot(freqs, 20 * np.log10(spectrum), color='#702963')
    ax5.set_xlabel("Frequency")
    ax5.set_ylabel("Magnitude (dB)")
    ax5.set_title("Spectrum Plot")
