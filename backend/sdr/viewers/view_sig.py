...
def view_sig(args):
    ...
    gs = gridspec.GridSpec(3, 2)  # Grid layout: 3 rows, 2 columns
    ax1 = plt.subplot(gs[0, :])  
    ax3 = plt.subplot(gs[1, :])  
    ax4 = plt.subplot(gs[2, 0])  #Third row, first half
    ax5 = plt.subplot(gs[2, 1])  #Third row, second half
    ...
    print("generate psd")
    spectrum_slice_samples = int(plotsamples/10)
    epsilon = 1e-10
    spectrum = np.abs(fftshift(fft(iqsslicecomplex[:spectrum_slice_samples])))
    freqs = np.linspace(-0.5, 0.5, len(iqsslicecomplex[:spectrum_slice_samples]))

    # Use semilogy for the y-axis
    ax5.semilogy(freqs, spectrum + epsilon, color='#702963')
    ax5.set_xlabel("Frequency")
    ax5.set_ylabel("Magnitude")
    ax5.set_title("Spectrum Plot with Semilog Y-axis")


    #quick and dirty constellation plot
    # # EXPERIMENTAL Rotate IQ data to roughly align phases
    # mean_phase = np.mean(np.angle(iqsslicecomplex))
    # iqsslicecomplex = iqsslicecomplex * np.exp(-1j*mean_phase)
    print("generate constellation")
    ax4.scatter(iqsslicecomplex.real, iqsslicecomplex.imag, color='#702963', s=1)
    ax4.set_xlabel("In-phase (I)")
    ax4.set_ylabel("Quadrature (Q)")
    ax4.set_title("Constellation Plot")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--png", type=bool, default=False, help="Save plot as PNG.", action=argparse.BooleanOptionalAction)
