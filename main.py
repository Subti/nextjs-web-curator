...
async def result(
    ...
    if action == "discard":
        ...
        if cuts and protocol and filename and num_samples and sample_rate:
            ...
            all_files = [
                str(pathlib.Path(dp) / f)
                for dp, dn, filenames in os.walk(pathlib.Path("slice_review"))
                for f in filenames
                if pathlib.Path(f).suffix in [".png", ".svg"]
            ]
            print(f"All files:{all_files}")
            return templates.TemplateResponse(
                html_filename, {"request": request, "recordings": all_files}
            )
